...
export async function updateDailyMessage(message: string, today: string): Promise<{ error: string } | undefined> {
  ...
  // The ENTIRE block is wrapped so any push/DB failure here can never fail the
  // daily-message save — the profiles UPDATE above has already committed, and a
  // buzz is a best-effort side-effect, not part of the save's success contract.
  if (trimmed) {
    try {
      ...
      console.error('[updateDailyMessage] buzz side-effect failed (save was unaffected)', e)
    }
  }
